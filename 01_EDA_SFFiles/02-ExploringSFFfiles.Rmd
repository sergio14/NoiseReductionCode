...
### Proporción de muestras unicas en los primeros 30 nucleótidos
```{r  message=FALSE, warning=FALSE}
tmpNames<-vector()
tmpCount<-vector()
tmpCount<-vector()
for(i in samples){
  sff<-readSff(paste("samples/",i,sep=""), use.qualities=TRUE, use.names=TRUE,clipMode = c("raw"), verbose=FALSE)
customClip(sff) <- IRanges(start = 1, end = 30)
clipMode(sff) <- "custom"
...
tmpNames<-c(tmpNames,gsub("454Reads.MID_","",gsub(".sff","",i)))
tmpCount<-c(tmpCount,length(table(counts=as.character(sread(sff)))))

}
as.data.frame(cbind(tmpNames,tmpCount))
